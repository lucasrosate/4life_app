...
import { createAccount, resetFormStatus } from '../../store/actions/userActions';
...
const RegistrationForm: React.FC<Props> = (props: Props) => {
    ...
    const handleShowLoginFormFunction = (responseMessage: any) => props.showLoginFormFunction(responseMessage);

    return (
        <Container width={"350px"} height={"750px"}>
            <form onSubmit={onSubmit} className={style.form}>
                <div className={style.signupFormContainer}>
                    <div>
                        <h1>Registrar</h1>
                        <label htmlFor="firstname"><span className={style.signupLabel}>Primeiro nome</span></label>
                        <input type="text" className={`input ${style.inputRegistration}`} placeholder="Primeiro Nome" name="firstname" ref={register({ required: true, maxLength: 80 })} />

                        <label htmlFor="lastname"><span className={style.signupLabel}>Sobrenome</span></label>
                        <input type="text" className={` input ${style.inputRegistration}`} placeholder="Sobrenome" name="lastname" ref={register({ required: true, maxLength: 100 })} />

                        <label htmlFor="username"><span className={style.signupLabel}>Nome de usuário</span></label>
                        <input type="text" className={`input ${style.inputRegistration}`} placeholder="Nome de usuário" name="username" ref={register({ required: true, maxLength: 80 })} />

                        <label htmlFor="email"><span className={style.signupLabel}>E-mail</span></label>
                        <input type="text" className={`input ${style.inputRegistration}`} placeholder="Email" name="email" ref={register({ required: true, pattern: /^\S+@\S+$/i })} />
                        ...
                        <input type="password" className={`input ${style.inputRegistration}`} placeholder="Senha" name="password1" onBlur={validatePassword} ref={register({ required: true, maxLength: 24 })} />

                        <label htmlFor="password2"><span className={style.signupLabel}>Repita a senha</span></label>
                        <input type="password" className={`input ${style.inputRegistration}`} placeholder="Senha" name="password2" onBlur={validatePassword} ref={register({ required: true, maxLength: 24 })} />

                        <div className="password-error" style={{ color: "rgb(240, 61, 61)", width: "300px", textAlign: "justify" }}>{passwordError}</div>
                        ...
                        <div style={{ color: "rgb(240, 61, 61)", marginTop: "10px", width: "300px", textAlign: "justify" }}>{props.responseMessage}</div>
